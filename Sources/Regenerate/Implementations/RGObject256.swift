import Foundation
import Bedrock
import TMap

public struct RGObject256<RootType: CID>: Codable where RootType.Digest == UInt256 {
    private let rawRoot: RootType!
    private let rawPaths: TMap<RootType.Digest, [Path]>!
}

extension RGObject256: RGObject {
    public var root: RootType { return rawRoot }
    public var keyPaths: TMap<RootType.Digest, [Path]> { return rawPaths }
    
    public init(root: RootType, paths: TMap<RootType.Digest, [Path]>) {
        self.rawRoot = root
        self.rawPaths = paths
    }
}
